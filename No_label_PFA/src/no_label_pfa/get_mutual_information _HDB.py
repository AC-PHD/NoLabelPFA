...
def get_mutual_information_HDB(path_original_data, path_principal_features_cluster_differences="principal_features_cluster_differences.txt", path_labels="hdbscan_labels.csv", path_feature_names="",clusters=[], number_output_functions=1, basis_log_mutual_information=2):
    ...
    for i in range(len(list_of_data_frames)):
        list_of_data_frames[i].to_csv("mutual_information_HDB"+str(i)+".csv")
